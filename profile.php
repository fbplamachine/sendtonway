...
<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.1.0/jquery.min.js"></script>
	...
	<title><?php echo $nom_profil.' '.$prenom_profil ?> - SendTonWay.com</title>
            ...
            if (!$result) die('Could not query');
                ...
                echo '<div class="annonce" style="width:550px;">
                        <div class="titrec"><i class="fas fa-box"></i>&nbsp;&nbsp;Annonce Colis</div>
                            ...
                            <label class="lab">Ville de départ:</label><p class="contenu">'.$ligne['lieux_depart'].'</p>
                            <label class="lab">Ville d\'arrivée:</label><p class="contenu">'.$ligne['lieux_arrive'].'</p><br>
                ...
                if ($ligne_c['demande_spec']) echo '<label class="lab">Demande speciale:</label><p class="contenu">'.$ligne_c['demande_spec'].'</p><br>';
                            echo '<label class="lab2">Prix: '.$ligne_c['tarif'].' FCFA</label><br><br></div>';
            }
    ...
    while (($ligne_c)||($ligne_t)) {
        if (isset($ligne_c['date_annonce'])>isset($ligne_t['date_annonce'])) {
            ...
            if (!$result) die('Could not query');
                ...
                echo '<div class="annonce">
                        <div class="titrec"><i class="fas fa-box"></i>&nbsp;&nbsp;Annonce Colis</div>
                            ...
                            <label class="lab">Ville de départ:</label><p class="contenu">'.$ligne['lieux_depart'].'</p>
                            <label class="lab">Adresse de départ:</label><p class="contenu">'.$ligne_c['adr_depart'].'</p><br>
                            <label class="lab">Ville d\'arrivée:</label><p class="contenu">'.$ligne['lieux_arrive'].'</p>
                            <label class="lab">Adresse d\'arrivée:</label><p class="contenu">'.$ligne_c['adr_arrive'].'</p><br>
                            <label class="lab">Taille:</label><p class="contenu">'.$ligne_c['taille'].'</p><br>
                            <label class="lab">Poids:</label><p class="contenu">'.$ligne_c['poids'].' KG</p><br>';
                if ($ligne_c['demande_spec']) echo '<label class="lab">Demande speciale:</label><p class="contenu">'.$ligne_c['demande_spec'].'</p><br>';
                echo '<label class="lab2">Prix: '.$ligne_c['tarif'].' FCFA</label><br><br>';
                ...
                if (mysqli_num_rows($r)>0) echo '<b style="margin-left:20px; color:#E02D05;">Ce colis a été demandé par quelqu\'un. Vérifiez vos notifications pour en savoir plus.</b><br><br>';
                echo '</div>';
            }
            $ligne_c=mysqli_fetch_array($result_c);
        } else {
            ...
            echo '<label class="tlab">Taille Max:</label><p class="contenu">'.$ligne_t['taille_max'].'</p>
                ...
                if (mysqli_num_rows($r)>0) echo '<b style="margin-left:10px; color:#187cc2;">Ce trajet a été demandé par quelqu\'un. Vérifiez vos notifications pour en savoir plus.</b><br>';
            echo '<br></div>';
    ...
    while (($ligne_c)||($ligne_t)) {
        if ($ligne_c['date_annonce']>$ligne_t['date_annonce']) {
            ...
            while ($ligne=mysqli_fetch_array($result)) {
                ...
                echo '<label class="lab2">Prix: '.$ligne['tarif'].' FCFA</label><br><br>';
            }
            echo '</div>';
            $ligne_t=mysqli_fetch_array($result_t);
        }
                    ...
                    if ($t['id_compte']==$_SESSION['id_compte']){
                        ...
                        echo '<div id="n'.$ligneu['id_compte'].'" style="width: 400px; height: 250px; background-color: #fff; border-radius: 20px; position: fixed; top:50%;left:50%; transform: translate(-50%,-50%); z-index:3;">
                            ...
                            <div style="float:left; margin-left:75px; margin-top:10px;">
                                ...
                                <a href="#" onclick="nn('.$ligneu['id_compte'].')" style="color:#E02D05; margin-left:70px;">Non, je ne veux pas noter cette personne.</a>
                        </div>';
                    }
                }
            }
...
<script type="text/javascript">
    $(document).ready(function page(){
        ...
        var css=".tabs ul li a{background:#E02D05;} .tabs ul li a:hover{background:#fff;}";
            if (der==1) {
                ...
                $("#infos").fadeIn();
                document.getElementById("tab_infos").style="background: #fff; color: #E02D05; border-bottom: 3px solid #E02D05;";
            ...
            if (der==2) {
                ...
                document.getElementById("tab_annonces").style.color="#E02D05";
            ...
            if (der==3) {
                ...
                document.getElementById("tab_demandes").style.color="#E02D05";
                document.getElementById("tab_annonces").style=css;
                document.getElementById("tab_historique").style=css;
                document.getElementById("tab_infos").style=css;
            }
            if (der==4) {
                ...
                document.getElementById("tab_historique").style.color="#E02D05";
                document.getElementById("tab_annonces").style=css;
                document.getElementById("tab_infos").style=css;
                document.getElementById("tab_demandes").style=css;
            }
