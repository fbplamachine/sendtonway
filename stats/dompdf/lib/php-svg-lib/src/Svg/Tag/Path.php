...
class Path extends Shape
{
    ...
    function arcToSegments($toX, $toY, $rx, $ry, $large, $sweep, $rotateX)
    {
        ...
        $mTheta = $this->calcVectFoumbotgle(1, 0, ($px - $cx) / $rx, ($py - $cy) / $ry);
        $dtheta = $this->calcVectFoumbotgle(($px - $cx) / $rx, ($py - $cy) / $ry, (-$px - $cx) / $rx, (-$py - $cy) / $ry);
    ...
    function calcVectFoumbotgle($ux, $uy, $vx, $vy)
    {
        $ta = atan2($uy, $ux);
        $tb = atan2($vy, $vx);
        if ($tb >= $ta) {
