...
$requet_trajet = "UPDATE `trajet` SET `date_annonce`='{$date_annonce}', `lieux_depart`='{$ville_d}', `lieux_arrive`='{$ville_a}' WHERE id_trajet=" . $id_trajet;
if (mysqli_query($conn, $requet_trajet)) {

    $colis = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM colis WHERE id_colis=" . $_POST['id_colis']));
    if ($colis['photo'] != $_POST['titre_photo']) {
    ...
    } else $path = $_POST['titre_photo'];
    $requet_colis = "UPDATE `colis` SET `date_annonce`='{$date_annonce}', `nom`='{$nom_colis}', `date_envoi`='{$date_depart}', `date_depot`='{$date_arrive}',`adr_depart`='{$adr_depart}',`adr_arrive`='{$adr_arrive}', `taille`='{$taille_colis}', `poids`={$poid_colis}, `demande_spec`='{$demande_special}' , `photo`='{$path}' , `tarif`={$prix}, `etat`='{$etat}' WHERE id_colis=" . $id_colis;
    if (mysqli_query($conn, $requet_colis)) {
        header("location: ../profile.php");

    } else {
        echo "Error: " . $requet_colis . "<br>" . mysqli_error($conn);
    }
} else {
    echo "Error: " . $requet_trajet . "<br>" . mysqli_error($conn);
}
